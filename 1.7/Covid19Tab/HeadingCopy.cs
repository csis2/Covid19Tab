...
namespace Covid19Tab
{
    ...
    // Copia o cabecalho do arquivo 'temp_covid19.txt'.
    {
        public void headingCopy()
        {
            ...
            {
                string lineswitch2 = File.ReadLines("C:\\Covid19Tab\\1.7\\Covid19Tab\\tmp\\temp_covid19.txt").First();
                using (StreamWriter sw = File.CreateText("C:\\Covid19Tab\\1.7\\Covid19Tab\\tmp\\heading.txt"))
                {
            ...
            if (!File.Exists(@"C:\covid19tab\1.7\covid19tab\tmp\heading.txt"))
            {
                Program.nErro = 1;
